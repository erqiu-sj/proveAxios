 ...
 * @returns
 */
export function dynamicModule(conf: Omit<dynamicPluginConfig, 'interceptor' | 'installer'>) {
 ...
 * @returns
 */
export function dynamicModuleSuccessInstall<T extends decisionInstaller.installReqSuc | decisionInstaller.installResSuc>(type: T) {
  return (target: Object, key: string, desc: TypedPropertyDescriptor<checkDynamicModuleSuccessInstallType<T>>) => {
    bindingSuccessInstaller(target, type, desc.value as checkDynamicModuleSuccessInstallType<T>)
    return desc
  }
}
